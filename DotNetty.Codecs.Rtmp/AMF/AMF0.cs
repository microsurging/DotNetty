...
namespace DotNetty.Codecs.Rtmp.AMF
{
	public class AMF0
	{
		...
		private static string DecodeString(IByteBuffer byteBuffer)
		{
            ...
            Memory<byte> bytes =new Memory<byte>(new byte[size]);
			byteBuffer.ReadBytes(bytes);
			return Encoding.UTF8.GetString(bytes.Span); // should we force encode to UTF-8 ?
		}
		...
		private static Object Decode(IByteBuffer byteBuffer, AMF0_Type type)
		{
			switch (type)
			{
				...
				case AMF0_Type.LONG_STRING:
					...
					Memory<byte> bytes = new Memory<byte>(new byte[stringSize]);
					byteBuffer.ReadBytes(bytes);
